...
router.get("/filter", async (req, res) => {
  ...
  const baseFilters = [{
    _id: {
      $nin: [mongoose.Types.ObjectId(privateId)]
    }
  }, {
    id: {
      $nin: restriction.blockBy
    }
  }, {
    id: {
      $nin: restriction.hide
    }
  }, {
    id: {
      $nin: restriction.block
    }
  }];
  let docs;
...
router.get("/read/item/:memberId/:itemId", (req, res) => {
  ...
  const checkFavAndView = Activity.findOne({
    privateId
  });
  const checkHide = Restriction.findOne({
    privateId
  });
  ...
  const getReviewData = Review.findOne({
    id: memberId
  }, {
    numOfReviews: 1,
    totalRating: 1
  });
  Promise.all([checkFavAndView, checkHide, getTwoOtherItems, getReviewData]).then(async docs => {
    ...
    if (!view) {
      ...
      Promise.all([updateActivityView, updateListingView]).then(docs => {
        res.json({
          fav,
          hide,
          twoOtherItems,
...
router.get("/read/items", async (req, res) => {
  ...
  const query = memberId ? {
    ...
    _id: mongoose.Types.ObjectId(privateId)
  };
...
router.patch("/update/:itemId", async (req, res) => {
  const data = req.body;
  const changes = Object.keys(req.body).reduce((acc, cur) => Object.assign(acc, {
    [`items.$.${cur}`]: data[cur]
  }), {});
  const {
    itemId
